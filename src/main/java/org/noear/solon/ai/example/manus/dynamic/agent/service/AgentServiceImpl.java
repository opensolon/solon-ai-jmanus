...
import org.noear.solon.ai.example.manus.agent.BaseAgent;
...
import org.noear.solon.ai.example.manus.dynamic.agent.ToolCallbackProvider;
import org.noear.solon.ai.example.manus.dynamic.agent.entity.DynamicAgentEntity;
import org.noear.solon.ai.example.manus.dynamic.agent.model.Tool;
...
import org.noear.solon.ai.example.manus.planning.PlanningFactory.ToolCallBackContext;
...
public class AgentServiceImpl implements AgentService {
	...
	private void updateEntityFromConfig(DynamicAgentEntity entity, AgentConfig config) {
		...
		if (!toolSet.contains(org.noear.solon.ai.example.manus.tool.TerminateTool.name)) {
			log.info("为Agent[{}]添加必要的工具: {}", config.getName(),
					org.noear.solon.ai.example.manus.tool.TerminateTool.name);
			toolSet.add(org.noear.solon.ai.example.manus.tool.TerminateTool.name);
		}
		// 3. 转为 List 并设置
